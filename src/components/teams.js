...
class Teams extends Component{
    ...
    searchTeam(event){
        ...
        if(keyword !== ''){
            const list = this.state.teams.filter((item)=>{
               return item.name.toLowerCase().indexOf(keyword.toLowerCase()) > -1; // y3ny akbr mn zero y3ny lw l2a 7aga
            })
            this.setState({ 
                filtered:list,
    ...
    renderList = ({filtered}) => { // {filtered} 3shan ygbly ely f setState
